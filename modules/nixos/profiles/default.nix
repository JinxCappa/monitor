...
in
{
  ...
  config = mkIf cfg.enable (mkMerge [
    # ============ BASE CONFIGURATION ============
    {
      ...
      # NetBird 0.76's plain-text status includes "0/0 Connected" while the
      # daemon still needs login. The upstream login unit mistakes that peer
      # count for a connected daemon and exits without using the setup key.
      systemd.services.netbird-wt0-login.preStart = ''
        get_status() {
          '${lib.getExe config.services.netbird.clients.wt0.wrapper}' status --detail 2>&1 || :
        }

        until get_status | grep --quiet -E '^Management: Connected|^Daemon status: NeedsLogin$'; do
          sleep 1
        done

        if get_status | grep --quiet '^Daemon status: NeedsLogin$'; then
          echo "Using configured NetBird setup key file" >&2
          '${lib.getExe config.services.netbird.clients.wt0.wrapper}' up \
      ...
      # NetBird's SSH integration needs this mutable OpenSSH drop-in
      # directory. ReadWritePaths fails the service with 226/NAMESPACE when
      # its target does not exist, which is the case on a fresh installation.
        ...
        text = ''
          ...
          generate_age_key() {
            ...
            if [ ! -f "$age_keys" ]; then
              ...
              ${pkgs.coreutils}/bin/install -d -m 700 "$age_dir"

              # nixos-anywhere sets TMPDIR to an installer-owned directory
              # below /mnt. That directory can be gone by the time target
              # activation scripts run, so keep the temporary key beside its
              # final destination instead of relying on the inherited TMPDIR.
              tmp_keys="$(${pkgs.coreutils}/bin/mktemp --tmpdir="$age_dir" .keys.txt.XXXXXXXXXX)"
              if ! ${pkgs.ssh-to-age}/bin/ssh-to-age \
                -private-key \
                -i /etc/ssh/ssh_host_ed25519_key > "$tmp_keys"; then
                ${pkgs.coreutils}/bin/rm -f "$tmp_keys"
                return 1
              fi
