...
in
{
  # Disable upstream module and replace with our extended version
  disabledModules = [ "services/networking/netbird.nix" ];

  imports = [
    # Import the upstream module from nixpkgs
    "${pkgs.path}/nixos/modules/services/networking/netbird.nix"
  ];

  # Extend the client options with SSH settings
