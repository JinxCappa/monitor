...
in
{
  ...
  systemd.services.check-ssh-key = {
    description = "Check and potentially regenerate SSH Host Keys (ED25519 & RSA)";
    before = [ "sshd.service" ];
  ...
  services.zabbixAgent2 = {
    ...
    # Note: Remote commands are enabled via AllowKey above, not plugin settings
    # plugins.settings = { };
