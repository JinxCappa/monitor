...
# Prepend FILTER_PATH to PATH for git filter subprocesses (git sanitizes env)
...
if [ -z "${HOME:-}" ]; then
	...
	if [ "$current_user" = "root" ]; then
		if ! SOPS_AGE_KEY=$(cat /etc/ssh/ssh_host_ed25519_key | ssh-to-age -private-key); then
			echo "Error: Failed to derive age key using ssh-to-age" >&2
			exit 1
		fi
		export SOPS_AGE_KEY
	else
...
init() {
	...
	git config --local "diff.${git_filter_name}.textconv" "$self_path clean '%f'"
...
update() {
	...
	for i in "$@"; do
		if ! exists_in_head "$i"; then
			...
			if ! "$sops_bin" --config "$sops_config" updatekeys "$i"; then
...
if ! command -v "$sops_bin" &> /dev/null; then
	echo "Not found: $sops_bin"
	exit 1
fi
